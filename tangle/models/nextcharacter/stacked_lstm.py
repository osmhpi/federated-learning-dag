...
from ..utils.language_utils import letter_to_vec_nc, word_to_indices_nc
...
class ClientModel(Model):
    ...
    def process_x(self, raw_x_batch):
        x_batch = [word_to_indices_nc(word) for word in raw_x_batch]
        x_batch = np.array(x_batch)
        return x_batch

    def process_y(self, raw_y_batch):
        y_batch = [letter_to_vec_nc(c) for c in raw_y_batch]
        return y_batch
